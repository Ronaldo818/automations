...
export default class FvProductPriceAndQuantitySelector extends LightningElement {
    ...
    handleNegotiatedPrice(event) {
        if (event.target.value == null || event.target.value == undefined || event.target.value == '') {
            event.target.value = 0;
        }
        this.negotiatedPrice = event.target.value;

    }

    handleDiscountPercentage(event) {
        if (event.target.value == null || event.target.value == undefined || event.target.value == '') {
            event.target.value = this.productPricing.unitPrice;
        }
    ...
    get editPriceDisabled() {
        return this.disablePriceEdit || this.isProcessing
    }
