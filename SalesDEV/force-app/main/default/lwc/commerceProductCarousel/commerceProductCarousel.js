...
/*import addToCartLabel from '@salesforce/label/c.addToCartAriaLabel';*/
...
export default class CommerceProductCarousel extends NavigationMixin(LightningElement) {
    ...
    wiredGetProducts({ error, data }) {
        if (data) {
            this.listaProdutos = JSON.parse(data);
            this.listaProdutos = this.listaProdutos.map((element) => ({
                ...element,
    ...
    async connectedCallback() {
        ...
        // console.log('this.effectiveAccountId1', this.effectiveAccountId  );
    ...
    handleOnMouseEnter(event){
        // console.log('event on enter:', event);
        event.toElement.classList.add('mouse-is-over');
    }   

    handleOnMouseLeave(event){
        // console.log('event on leave:', event);
